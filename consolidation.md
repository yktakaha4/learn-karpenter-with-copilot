...
>     // 複数ノードの統合の場合:
    // 残りのインスタンスタイプの柔軟性を確認する必要がないため、このケースでは早期に終了します

>     // 単一ノードの統合の場合:
    // 現在の候補インスタンスよりも安価なインスタンスが15個あるかどうかを確認します。この場合、以下のことが分かります:
    //   1) 現在の候補インスタンスは最も安価な15個のインスタンスタイプのセットに含まれていない。
    //   2) 現在の候補インスタンスよりも安価なオプションが少なくとも15個存在している。

### 5. インスタンスタイプの制限
- インスタンスタイプの選択肢を15に制限します。
- これにより、繰り返しの統合を防ぎます。

>     // ユーザーがNodePoolの要件でminValuesを設定している場合、spot-to-spot統合を有効にするために送信されるインスタンスタイプの数を100に制限します。
    // NodePoolの要件にminValuesが設定されていない場合、デフォルトの15を使用してインスタンスタイプを制限し、spot-to-spot統合を有効にします。
    // デフォルト値（15）の場合、インスタンスタイプを15に制限することで、継続的な統合の状況に陥らないようにします。
    // 例えば：
    // 1) 価格順に並んだ5つのインスタンスタイプ (A, B, C, D, E) があり、最小柔軟性が3で、これらすべてがポッドに適合するとします。CreateInstanceFromTypes(A,B,C,D,E) を送信し、価格とスポットの可用性に基づいてEタイプが返されます。
    // 2) Eが (A,B,C,D) の一部でないことを確認し、Eより安い (A,B,C,D) を使用してすぐに統合を再送信します。
    // 3) CreateInstanceFromTypes(A,B,C,D) がDを返したと仮定し、Dが (A,B,C) の一部でないことを確認し、Dより安い (A,B,C) を使用して再び統合を送信します。この結果、継続的な統合が発生します。
    // ステップ(1)でインスタンスタイプを最小柔軟性に制限していれば、CreateInstanceFromTypes(A,B,C) を送信し、リストの一部であるインスタンスタイプを受け取ることで、即時の統合を防ぐことができました。
    // これを15タイプに拡張し、CreateInstanceFromTypes呼び出しで最も安い15タイプのみを送信する必要があります。これにより、結果として得られるインスタンスタイプが常にその15タイプのセット内にあり、即時の統合が発生しなくなります。
